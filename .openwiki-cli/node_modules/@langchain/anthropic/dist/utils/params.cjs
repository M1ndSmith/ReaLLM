...
const FABLE_MODEL_PREFIXES = ["claude-fable-5"];
...
function validateInvocationParamCompatibility(fields) {
	const { model, thinking, thinkingExplicitlySet, outputConfig, topK, topP, temperature } = fields;
	...
	if (fableModel && thinkingExplicitlySet && thinking.type === "disabled") throw new Error(`thinking.type="disabled" is not supported for ${modelName}; omit thinking to use adaptive thinking instead`);
