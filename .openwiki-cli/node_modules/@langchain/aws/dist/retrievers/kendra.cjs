...
let _aws_sdk_client_kendra = require("@aws-sdk/client-kendra");
...
* ```
*/
var AmazonKendraRetriever = class extends _langchain_core_retrievers.BaseRetriever {
	static lc_name() {
	...
	constructor({ indexId, topK = 10, clientOptions, attributeFilter, region }) {
		...
		this.topK = topK;
		this.kendraClient = new _aws_sdk_client_kendra.KendraClient({
			region,
			...clientOptions
		});
	...
	*/
	cleanResult(resText) {
		return resText.replace(/\s+/g, " ").replace(/\.\.\./g, "");
	}
	/**
	...
	*/
	convertRetrieverItem(item) {
		...
		const metadata = {
			source: item.DocumentURI,
			title,
			excerpt,
			document_attributes: this.getDocAttributes(item.DocumentAttributes)
		};
	...
	*/
	async queryKendra(query, topK, attributeFilter) {
		const retrieveCommand = new _aws_sdk_client_kendra.RetrieveCommand({
		...
		if (retrieveResponse.ResultItems?.length === 0) {
			const queryCommand = new _aws_sdk_client_kendra.QueryCommand({
				IndexId: this.indexId,
				QueryText: query,
				PageSize: topK,
				AttributeFilter: attributeFilter
	...
	async _getRelevantDocuments(query) {
		return await this.queryKendra(query, this.topK, this.attributeFilter);
	}
};
//#endregion
