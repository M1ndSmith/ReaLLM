...
function convertToConverseTools(tools) {
	return tools.map((tool, index) => {
		if ((0, _langchain_core_language_models_base.isOpenAITool)(tool)) return { toolSpec: {
			name: tool.function.name,
			description: tool.function.description,
			inputSchema: { json: tool.function.parameters }
		} };
		else if ((0, _langchain_core_utils_function_calling.isLangChainTool)(tool)) return { toolSpec: {
			name: tool.name,
			description: tool.description,
			inputSchema: { json: (0, _langchain_core_utils_types.isInteropZodSchema)(tool.schema) ? (0, _langchain_core_utils_json_schema.toJsonSchema)(tool.schema) : tool.schema }
		} };
...
function convertToBedrockToolChoice(toolChoice, tools, fields) {
	...
	if (typeof toolChoice === "string") switch (toolChoice) {
		...
		case "auto":
			...
			break;
		default:
			if (!tools.find((tool) => tool.toolSpec?.name === toolChoice)) throw new Error(`Tool with name ${toolChoice} not found in tools list.`);
...
function supportedToolChoiceValuesForModel(model) {
	if (model.includes("claude-3") || model.includes("claude-4") || model.includes("claude-haiku-4") || model.includes("claude-opus-4") || model.includes("claude-sonnet-4") || model.includes("claude-haiku-5") || model.includes("claude-opus-5") || model.includes("claude-sonnet-5")) return [
		"auto",
		"any",
		"tool"
	];
