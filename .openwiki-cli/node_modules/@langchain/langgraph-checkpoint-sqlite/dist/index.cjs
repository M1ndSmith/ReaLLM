  ...
  PRIMARY KEY (thread_id, checkpoint_ns, checkpoint_id, task_id, idx)
	...
	async *list(config, options) {
      ...
      FROM checkpoints\n`;
		...
		const sanitizedFilter = Object.fromEntries(Object.entries(filter ?? {}).filter(([, value]) => value !== void 0));
		...
		const args = [
			...
			...Object.entries(sanitizedFilter).flatMap(([key, value]) => [`$.${key}`, JSON.stringify(value)])
	...
	async putWrites(config, writes, taskId) {
		...
		const allSpecial = writes.every(([channel]) => channel in _langchain_langgraph_checkpoint.WRITES_IDX_MAP);
    ...
    `);
		this.db.transaction((rows) => {
			...
			return [
				...
				taskId,
				_langchain_langgraph_checkpoint.WRITES_IDX_MAP[write[0]] ?? idx,
				write[0],
				type,
				serializedWrite
			];
