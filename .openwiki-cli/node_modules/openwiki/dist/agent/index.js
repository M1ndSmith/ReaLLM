...
export function createModel(provider, modelId, providerRetryAttempts, maxOutputTokens, streamIdleTimeout) {
    ...
    const chatOpenAiUsesResponsesApi = providerUsesResponsesApi(provider, modelId);
    const reasoningConfig = resolveReasoningConfig(provider, modelId, process.env, {
    ...
    const chatCompletionsReasoningOptions = reasoningConfig?.transport === "chat-completions-reasoning-effort"
        ...
        : {};
    const geminiThinkingLevelOptions = reasoningConfig?.transport === "gemini-thinking-level"
    ...
    const baseURL = resolveProviderBaseUrl(provider);
    const configuration = provider === "openai-compatible" && !chatOpenAiUsesResponsesApi
        ? {
            ...(baseURL ? { baseURL } : {}),
...
function createOpenAiCompatibleFetch() {
    return (input, init) => globalThis.fetch(input, normalizeOpenAiCompatibleChatCompletionsInit(input, init) ?? init);
}
function normalizeOpenAiCompatibleChatCompletionsInit(input, init) {
    if (!isOpenAiCompatibleChatCompletionsRequest(input, init)) {
        return null;
    }
    const body = typeof init?.body === "string" ? init.body : null;
    const parsedBody = parseJsonRecord(body);
    const normalizedBody = parsedBody === null
        ? null
...
function isOpenAiCompatibleChatCompletionsRequest(input, init) {
...
function normalizeOpenAiCompatibleChatCompletionsBody(body) {
...
function normalizeOpenAiCompatibleTextContent(content) {
...
function isOpenAiCompatibleTextContentBlock(block) {
            ...
            while (true) {
                ...
                if (!response.ok) {
                    ...
                    if (attempt < retry.maxRetries &&
                        ...
                        for (const sink of activeOpenRouterSinks) {
                            emitDebug(sink.options, `openrouter.retry status=404 attempt=${attempt}/${retry.maxRetries} delayMs=${delayMs}`);
                        }
 ...
 */
export function installOpenRouterDebugFetch(options, retryAttempts = 0, fetchOptions = {}) {
    const sink = {
        lastFailure: null,
        options,
        retryAttempts,
...
function getOpenRouterProviderErrorRetryDelayMs(attempt) {
    return Math.min(OPENROUTER_PROVIDER_ERROR_RETRY_BASE_DELAY_MS * 2 ** (attempt - 1), OPENROUTER_PROVIDER_ERROR_RETRY_MAX_DELAY_MS);
}
function isOpenRouterRequestResendable(input, init) {
...
function isTransientOpenRouterProvider404(response, rawBody) {
...
async function readResponseBody(response) {
    try {
        ...
        return {
            preview: sanitizedBody.length <= OPENROUTER_DEBUG_BODY_LIMIT
                ? sanitizedBody
        ...
        return {
            preview: `Unable to read response body: ${error instanceof Error ? error.message : String(error)}`,
            raw: null,
        };
    }
}
