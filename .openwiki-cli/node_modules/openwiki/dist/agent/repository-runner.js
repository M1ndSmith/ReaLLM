...
import { AIMessage, AIMessageChunk, ChatMessage, ChatMessageChunk, ToolMessage, collapseToolCallChunks, defaultToolCallParser, } from "@langchain/core/messages";
...
function coerceRepositoryWorkerModelResponse(response) {
    ...
    if (ChatMessage.isInstance(candidate) &&
        isGenericAssistantModelResponse(candidate)) {
        const rawToolCalls = getOpenAiRawToolCalls(candidate.additional_kwargs);
        const toolCallFields = rawToolCalls === null ? {} : parseRawOpenAiToolCalls(rawToolCalls);
...
function isGenericAssistantModelResponse(response) {
    return response.role === undefined || response.role === "assistant";
}
function getOpenAiRawToolCalls(additionalKwargs) {
...
function parseRawOpenAiToolCalls(rawToolCalls) {
...
function toToolCallChunk(rawToolCall) {
    ...
    return {
        ...
        index: typeof rawToolCall.index === "number" ? rawToolCall.index : undefined,
        name: typeof functionFields.name === "string" ? functionFields.name : undefined,
        args: typeof functionFields.arguments === "string"
