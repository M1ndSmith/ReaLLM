...
export const OPENAI_COMPATIBLE_REASONING_EFFORT_SUPPORTED_ENV_KEY = "OPENWIKI_OPENAI_COMPATIBLE_REASONING_EFFORT_SUPPORTED";
...
export function providerUsesResponsesApi(provider, modelId, env = process.env) {
...
export function providerHasFixedModel(provider) {
    return getProviderConfig(provider).fixedModel !== undefined;
}
export function getProviderFixedModel(provider) {
...
export function resolveOpenAiCompatibleReasoningEffortSupported(env = process.env) {
    return (env[OPENAI_COMPATIBLE_REASONING_EFFORT_SUPPORTED_ENV_KEY]?.trim().toLowerCase() === TRUE_ENV_VALUE);
}
