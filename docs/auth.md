...
`GATEWAY_API_KEY` is optional inbound auth for this process. It is not a provider `*_API_KEY` and not a LiteLLM Proxy virtual key (no spend DB, no per-client budgets).
...
Missing or wrong key is HTTP 401 `{ "detail": { "error": "gateway_unauthorized" } }`. That is distinct from LiteLLM `AuthenticationError` (provider credentials), which is also 401 with a provider message.
...
`PATCH /config` additionally requires a configured gateway key (403 `gateway_key_required` when auth is off) so an open Compose port cannot flip MEMORY / PII / GUARD.
