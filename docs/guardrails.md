...
Chat can block prompt injection and unsafe content with Meta classifiers, called through LiteLLM's Router. Llama Guard 4 is this gateway's content moderator.

Pipeline order is in [architecture](architecture.md). Inbound guards run after Mem0 inject and the second Presidio pass. Llama Guard outbound runs after Presidio outbound.
